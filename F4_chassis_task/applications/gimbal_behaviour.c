...
void gimbal_no_follow_chassis_control(gimbal_control_t *gimbal_no_follow_chassis)
{
    ...
    int16_t ch1_set = -gimbal_no_follow_chassis->gimbal_RC->rc.ch[0] / 66;

    // float data[1];
    // data[0] = ch1_set;
    // VOFA_Transmit_JustFloat(data, 1);

    CAN_cmd_dm_j4310_mit(0.0f, ch1_set, 0.0f, 0, 0.5f);
}
...
void gimbal_follow_chassis_control(gimbal_control_t *gimbal_follow_chassis)
{
    ...
    float ch1_set = -(float)(gimbal_follow_chassis->gimbal_RC->rc.ch[0]) / 660.0f * PI;

    CAN_cmd_dm_j4310_mit(ch1_set, 0, 0.0f, 2.0f, 0.2f);

    //云台IMU的yaw轴,pitch轴,gyro_z,gyro_x
    fp32 position = gimbal_follow_chassis->motor_measure->position;
    while (position > PI) position -= 2 * PI;
    while (position < -PI) position += 2 * PI;

    fp32 yaw_angle_get = position - yaw_motor_offset;
...
void gimbal_rotation_control(gimbal_control_t *gimbal_rotation_control)
{
    ...
    float position = gimbal_rotation_control->motor_measure->position;
    while (position > PI) position -= 2 * PI;
    while (position < -PI) position += 2 * PI;
