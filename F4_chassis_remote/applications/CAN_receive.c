...
#define get_boards_measure_IMU(ptr, data)								  \
{																		  \
	int16_t raw_yaw = (int16_t)((data)[0] << 8 | (data)[1]);              \
	int16_t raw_gyro_z = (int16_t)((data)[2] << 8 | (data)[3]);           \
	(ptr)->yaw = (float)raw_yaw / 10000.0f;								  \
	(ptr)->gyro_z = (float)raw_gyro_z / 10000.0f;						  \
}

#define get_boards_measure_ch(ptr, data)								  \
{																		  \
	(ptr)->ch0 = (int16_t)((data)[0] << 8 | (data)[1]);                   \
	(ptr)->ch1 = (int16_t)((data)[2] << 8 | (data)[3]);                   \
	(ptr)->ch2 = (int16_t)((data)[4] << 8 | (data)[5]);                   \
	(ptr)->ch3 = (int16_t)((data)[6] << 8 | (data)[7]);                   \
}

#define get_boards_measure_s(ptr, data)									  \
{																		  \
	(ptr)->s0 = (int16_t)((data)[0] << 8 | (data)[1]);                    \
	(ptr)->s1 = (int16_t)((data)[2] << 8 | (data)[3]);                    \
}
 	...
 	if(hcan==&hcan1)
	...
	else if (hcan==&hcan2)
		{
			...
			switch (rx_header.StdId)
			{
				case IMU_ID:
					{
						get_boards_measure_IMU(&boards_measure, rx_data);
						break;
					}
				case REMOTE_ID_CH:
					{
						get_boards_measure_ch(&boards_measure, rx_data);
						break;
					}
				case REMOTE_ID_S:
					{
						get_boards_measure_s(&boards_measure, rx_data);
